...
#Preugnta a
#se importn el dataframe desde el csv
data = pd.read_csv('data/HIV.csv',sep=';',decimal=',', index_col = 0)
#se eliminan las primeras 11 columnas
data = data.drop(data.columns[range(0,11)], axis=1)
#Se mantienen todas las filas que tienen el dato 2010 como no nulo
data = data[data['2010'].notnull()]
#Se eliminan todas aquellas filas con datos nulos
data = data.dropna()
#Representa la dimensionalidad del dataframe, ademas de los nombres de los paises y los anos en tuplas
data.shape
data.index.names = ['country']
data.columns.names = ['year']

#Pregunta b
#Se eligen ciertor paises y se hace un line chart
...
#Pregunta c
#se mueven los valores del dataset a una matriz
